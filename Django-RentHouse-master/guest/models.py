...
class MessageModel(models.Model):
    renter = models.ForeignKey(AddroomModel, on_delete=models.CASCADE)
    name = models.CharField(max_length=150)
    email = models.CharField(max_length=150)
    subject = models.CharField(max_length=200)
    query = models.CharField(max_length=700)
...
class Prediction(models.Model):
    sellerchoice=[('OWNER','Owner'),('AGENT','Agent')]
    layoutchoice = [('BHK', 'with a salon'), ('RK', 'without a salon') ]
    propchoice = [('Apartment', 'Condominium'), ('Independent Floor', 'Floor'),
                  ('Independent House', 'House'), ('Studio Apartment', 'Studio'), ('Villa', 'Villa')]
    localchoice = [('Limat','Limat'), ('Sikela','Sikela'), ('Secha','Secha')]
    furnishchoice = [('Furnished','Furnished'),('Semi-Furnished','Semi-Furnished'),('Unfurnished','Unfurnished')]
    seller = models.CharField(max_length=20,choices=sellerchoice)
    bedroom = models.FloatField()
    layout = models.CharField(max_length=20, choices=layoutchoice)
    prop = models.CharField(max_length=20, choices=propchoice)
    locality = models.CharField(max_length=20, choices=localchoice)
    area = models.FloatField()
    furnish = models.CharField(max_length=20, choices=furnishchoice)
    bathroom = models.FloatField()

class Rented(models.Model):
    rent = models.ForeignKey(AddroomModel, on_delete=models.CASCADE)
    is_rented = models.BooleanField(default=False, blank=True, null=True)

    def __str__(self):
        return f'{self.rent.user} - Profile'
