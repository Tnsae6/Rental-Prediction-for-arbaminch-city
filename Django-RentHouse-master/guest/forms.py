...
from guest.models import ContactModel, Prediction, ProfilePic, AddroomModel, GuestReviewsModel, MessageModel, Rented
...
class AddroomForm(forms.ModelForm):
    ...
    def clean_troom(self):
        ...
        if troom >= 10:
            raise ValidationError('Please enter less than 10 room!')
        else:
            return troom
    def clean_price(self):
        price = int(self.cleaned_data.get('price'))
        if price <= 1000:
            raise ValidationError('Please enter greater than  Etb.1000/- birr ')
        else:
            return price
